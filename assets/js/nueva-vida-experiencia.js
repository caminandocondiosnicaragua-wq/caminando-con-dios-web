...
(function(){
  ...
  function eliminarDuplicadosVF_(){
    ...
    document.querySelectorAll('.nv1-section:not([data-step="1"]) .nv1-question').forEach(card=>{
      if(claves.has(normalizar(card.querySelector("h3")?.textContent))) card.remove();
    });
  }
  ...
  function transformarEnsenanzas_(){
    document.querySelectorAll(".nv1-db-teaching").forEach(card=>{
      if(card.querySelector(".nv1-step-list")) return;
      const p = card.querySelector("p");
      if(!p) return;
      const texto = p.textContent.trim();
      const partes = texto.split(/(?=\d+\.\s)/).map(x=>x.trim()).filter(Boolean);
      const pasos = partes.length > 1 ? partes : [texto];
      const wrap = document.createElement("div");
      wrap.className = "nv1-step-list";
      pasos.forEach((paso,i)=>{
        const m = paso.match(/^\d+\.\s*(.*)$/s);
        const contenido = m ? m[1] : paso;
        const el = document.createElement("div");
        el.className = "nv1-step-card";
        el.innerHTML = `<span>${i+1}</span><p>${escapeHTML_(contenido)}</p>`;
        wrap.appendChild(el);
      });
      p.replaceWith(wrap);
      if(!card.querySelector(".nv1-card-art")){
        const img = document.createElement("img");
        img.className = "nv1-card-art";
        img.src = "assets/img/nueva-vida/ensenanza.svg";
        img.alt = "Ilustración para acompañar la enseñanza";
        card.prepend(img);
      }
    });
    document.querySelectorAll(".nv1-db-reflection").forEach(card=>{
      if(card.querySelector(".nv1-card-art")) return;
      const img = document.createElement("img");
      img.className = "nv1-card-art nv1-card-art-reflection";
      img.src = "assets/img/nueva-vida/reflexion.svg";
      img.alt = "Persona en un momento de reflexión";
      card.prepend(img);
    });
  }
  ...
  function agregarAccesibilidad_(){
    ...
    bar.innerHTML = `<span>🔎 Tamaño de texto</span><button type="button" data-font="minus" aria-label="Reducir letra">A−</button><button type="button" data-font="normal" aria-label="Tamaño normal">A</button><button type="button" data-font="plus" aria-label="Agrandar letra">A+</button>`;
    progress.after(bar);
  ...
  function reproducirTexto_(texto, boton){
    if(!("speechSynthesis" in window)){ boton.textContent = "🔊 Audio no disponible"; return; }
  ...
  function agregarCamposDeEjercicio_(){
    document.querySelectorAll(".nv1-db-exercise").forEach(card=>{
      ...
      if(/fecha\s*:/i.test(card.textContent || "")){
  ...
  function mejorarExamen_(){
    const opciones = document.getElementById("nv1-game-options");
    if(!opciones) return;
    const barajar = ()=>{
      const botones = Array.from(opciones.querySelectorAll(".nv1-game-option"));
      ...
      estado.forEach((item,i)=>{
        item.node.textContent = `${String.fromCharCode(65+i)}. ${item.text}`;
        if(item.onclick) item.node.setAttribute("onclick",item.onclick);
        opciones.appendChild(item.node);
      });
    };
    const observer = new MutationObserver(()=>barajar());
    observer.observe(opciones,{childList:true});
    setTimeout(barajar,0);
  }
  ...
  function parsearReferenciaVersiculos_(referencia){
    ...
    if(!m) return null;
    const libro=m[1].trim(), codigo=obtenerCodigoLibro(libro);
    if(!codigo) return null;
    const capitulo=Number(m[2]), inicio=Number(m[3]||1), fin=Number(m[4]||inicio);
